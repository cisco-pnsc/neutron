...
class ServiceVMPlugin(vm_db.ServiceResourcePluginDb, ServiceVMMgmtMixin):
    ...
    def create_device_template(self, context, device_template):
        if not context.is_admin:
            msg='Unauthorized, only admin user can access'
        ...
        if not attributes.is_attr_set(tenantId):
	    ...
	    if self.isdevicetemplate_inTenant(context,service_type['service_type'],tenantId):
	...
	device_template=self.get_device_template(context, device_template_id)
        ...
        if service_type['service_type'] == 'lbaas':
            if self.isvip_inTenant(context,tenant_id):
                LOG.debug(_('service type exist in tenant'))
                raise servicevm.Serviceinuse(service_type=service_type['service_type'])
        return super(ServiceVMPlugin, self).delete_device_template(
    ...
    def get_device_template(self, context, device_template_id, fields=None):
        if not context.is_admin:
            msg='Unauthorized, only admin user can access'
            raise  webob.exc.HTTPBadRequest(msg)
        return super(ServiceVMPlugin, self).get_device_template(context, device_template_id, fields=None)        

    def get_device_templates(self, context, filters=None, fields=None):
        if not context.is_admin:
            msg='Unauthorized, only admin user can access'
            raise  webob.exc.HTTPBadRequest(msg)
        return super(ServiceVMPlugin, self).get_device_templates(context, filters, fields=None)
    ...
    def isvip_inTenant(self,context,tenant_id):
        ...
        for vip in vips:
            if vip['tenant_id']==tenant_id:
                return True
        return False
    def isdevicetemplate_inTenant(self,context,service_type,tenant_id):
	device_templates = self.get_device_templates(context,None)
        for servicetypes in device_templates:
            if servicetypes['tenant_id'] == tenant_id:
