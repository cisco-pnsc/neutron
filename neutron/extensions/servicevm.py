...
class DeviceTemplateNotFound(exceptions.NotFound):
    message = _('device template %(device_template_id)s could not be found')
...
class deplomentpolicyExists(exceptions.InUse):
    message = _('deployment policy is configured for the service type on tenant')

class Serviceinuse(exceptions.InUse):
    message = _('service instance %(service_type)s is in use on tenant')

class AttributeNotSpecified(exceptions.InvalidInput):
    message = _('Attribute  %(attr)s  not speicfied')

class AttributeCountMismatch(exceptions.InvalidInput):
    message = _('Create command supports 6 attributes enable_ha, admin_user, admin_password, platform, version, license_category')
...
class Servicevm(extensions.ExtensionDescriptor):
    ...
    def get_namespace(cls):
        return 'http://wiki.openstack.org/Neutron/ServiceVM'
    ...
    def get_resources(cls):
        ...
        plural_mappings['device_templates'] = 'device_template'
        ...
        return resource_helper.build_resource_info(
            ...
            register_quota=True, translate_name=True)
