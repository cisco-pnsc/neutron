...
class Device(model_base.BASEV2, models_v2.HasId, models_v2.HasTenant):
...
class DeviceArg(model_base.BASEV2, models_v2.HasId):
...
class DeviceServiceContext(model_base.BASEV2, models_v2.HasId):
...
class ServiceResourcePluginDb(servicevm.ServiceVMPluginBase,
    ...
    def _get_resource(self, context, model, id):
        try:
            ...
            if issubclass(model, DeviceTemplate):
                raise servicevm.DeviceTemplateNotFound(device_template_id=id)
    ...
    def create_device_template(self, context, device_template):
        ...
        isglobal= template.get('scope')
        if isglobal=='global':
            tenant_id = constants.GLOBAL_DEVICE_TEMPLATE_TID
        else:
